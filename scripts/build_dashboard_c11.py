...
h=[]
h.append("<html><head><meta charset='utf-8'><title>C11 PRO</title></head>")
...
h.append(f"<p>M17 tri={m17.get('tri')} quad={m17.get('quad')} weak=0</p>")
h.append("<h2>M19 Centrality</h2><table border=1 cellpadding=4>")
h.append("<tr><th>root</th><th>deg</th><th>wdeg</th><th>pr</th></tr>")
for r in m19[:15]:
 h.append(f"<tr><td>{r['root']}</td><td>{r['degree']}</td><td>{r['weighted_degree']}</td><td>{r['pagerank']}</td></tr>")
h.append("</table><h2>M20 PMI</h2><table border=1 cellpadding=4>")
h.append("<tr><th>r1</th><th>r2</th><th>co</th><th>pmi</th></tr>")
for r in m20[:15]:
 h.append(f"<tr><td>{r['root1']}</td><td>{r['root2']}</td><td>{r['cooccur']}</td><td>{r['pmi']}</td></tr>")
h.append("</table><h2>M21 KL 108=8.60 Meccan 3.03 Medinan 1.79</h2><table border=1 cellpadding=4>")
h.append("<tr><th>surah</th><th>KL</th><th>words</th></tr>")
for r in m21[:15]:
